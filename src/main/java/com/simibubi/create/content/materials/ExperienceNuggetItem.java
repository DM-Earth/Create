...
public class ExperienceNuggetItem extends Item {
	...
	public TypedActionResult<ItemStack> use(World pLevel, PlayerEntity pPlayer, Hand pUsedHand) {
		...
		if (pLevel.isClient) {
			pLevel.playSound(pPlayer, pPlayer.getBlockPos(), SoundEvents.BLOCK_AMETHYST_BLOCK_BREAK, SoundCategory.PLAYERS,
				.5f, 1);
			return TypedActionResult.consume(itemInHand);
		}
		...
		for (int i = 0; i < maxOrbs; i++) {
			...
			Vec3d offset = VecHelper.offsetRandomly(Vec3d.ZERO, pLevel.random, 1)
				.normalize();
			Vec3d look = pPlayer.getRotationVector();
			Vec3d motion = look.multiply(0.2)
				.add(0, 0.2, 0)
				.add(offset.multiply(.1));
			Vec3d cross = look.crossProduct(VecHelper.rotate(new Vec3d(-.75f, 0, 0), -pPlayer.getYaw(), Axis.Y));

			Vec3d global = pPlayer.getEyePos()
				.add(look.multiply(.5f))
				.add(cross);
