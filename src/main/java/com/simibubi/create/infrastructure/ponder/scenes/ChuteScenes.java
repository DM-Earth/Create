...
public class ChuteScenes {
	...
	public static void downward(SceneBuilder scene, SceneBuildingUtil util) {
		...
		scene.overlay.showControls(
			...
			40);
		scene.idle(7);
		scene.world.modifyBlock(util.grid.at(3, 3, 3), s -> s.with(ChuteBlock.SHAPE, ChuteBlock.Shape.WINDOW),
			false);
		...
		scene.overlay.showControls(
			...
			40);
		scene.idle(7);
		scene.world.modifyBlock(util.grid.at(3, 2, 3), s -> s.with(ChuteBlock.SHAPE, ChuteBlock.Shape.ENCASED),
			false);
		scene.overlay.showText(50)
		...
		scene.world.modifyBlock(util.grid.at(2, 3, 2), s -> s.with(SHAPE, Shape.INTERSECTION), false);
