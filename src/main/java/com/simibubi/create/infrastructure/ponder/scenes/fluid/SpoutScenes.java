...
public class SpoutScenes {
	...
	public static void filling(SceneBuilder scene, SceneBuildingUtil util) {
		...
		ItemStack bucket = new ItemStack(AllFluids.HONEY.get().getBucketItem());
//			.getAttributes()
//			.getBucket(honey);
		...
		ParticleEffect fluidParticle = FluidFX.getFluidParticle(new FluidStack(AllFluids.HONEY.get(), FluidConstants.BUCKET));
		for (int i = 0; i < 10; i++) {
			scene.effects.emitParticles(util.vector.topOf(depotPos.south())
				.add(0, 1 / 16f, 0),
				Emitter.simple(fluidParticle, VecHelper.offsetRandomly(Vec3d.ZERO, random, .1f)), 1, 1);
		}
		scene.idle(10);
