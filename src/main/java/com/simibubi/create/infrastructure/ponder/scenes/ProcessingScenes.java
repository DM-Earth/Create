...
public class ProcessingScenes {
	...
	public static void crushingWheels(SceneBuilder scene, SceneBuildingUtil util) {
		...
		scene.world.modifyEntity(entity1, Entity::discard);
		Emitter blockSpace =
			Emitter.withinBlockSpace(new ItemStackParticleEffect(ParticleTypes.ITEM, input), util.vector.of(0, 0, 0));
	...
	public static void emptyBlazeBurner(SceneBuilder scene, SceneBuildingUtil util) {
		...
		scene.world.createEntity(w -> {
			...
			blazeEntity.setYaw(blazeEntity.prevYaw = 180);
			return blazeEntity;
		});
		...
		scene.world.modifyEntities(BlazeEntity.class, Entity::discard);
		scene.idle(20);
		...
		scene.world.modifyBlock(util.grid.at(3, 1, 2),
			s -> s.with(LitBlazeBurnerBlock.FLAME_TYPE, LitBlazeBurnerBlock.FlameType.SOUL), false);
		scene.overlay.showText(60)
	...
	public static void basin(SceneBuilder scene, SceneBuildingUtil util) {
		...
		Vec3d filter = util.vector.of(2.5, 2.825, 2.5);
